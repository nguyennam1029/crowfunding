...
import { setQuery } from "store/campaign/campaign-slice";
...
const DashboardSearch = () => {
  ...
  const handlePutQuery = () => {
    setShowSearch(false);
    if (valueQuery) {
      dispatch(setQuery(valueQuery));

      navigate(`/campaign/search/${valueQuery}`);
      setValueQuery("");
      return;
    }
    toast.warning("Please enter keywords");
  };
  return (
    <div className="relative z-50">
      ...
      {showSearch && (
        <div className="search-results w-full lg:w-[843px] absolute top-full left-0 bg-white z-50 translate-y-5 pb-6 rounded-3xl transition duration-700 ease-in-out shadow-2xl">

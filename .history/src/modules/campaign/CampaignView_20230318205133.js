    ...
    try {
      async function featchData() {
        const { data } = await axios.get(
          // `https://rose-funny-greyhound.cyclic.app/campaigns/${id}`
          `/campaigns/${id}`
        );
        setCampaign(data);
      }
  ...
  useEffect(() => {
    try {
      async function featchData() {
        const { data } = await axios.get(
          "/categories"
          // "https://rose-funny-greyhound.cyclic.app/categories"
        );
        setCategories(data);
      }
      featchData();
    ...
    <Fragment>
      ...
      <div className="flex items-start gap-x-10 w-full max-w-[1066px]">
        <div className="flex-1">
          <CampImage
            imageUrl={campaign?.featured_image?.url}
      ...
      <div className="flex items-center justify-between mt-[100px] bg-white py-5 border-b border-b-slate-100 mb-6">
          ...
          {categories.length > 0 &&
            categories.map((item) => (
              <NavLink
                ...
                key={item?.title}
                className={({ isActive }) =>
                  isActive ? ` text-secondary ` : ` text-text3`
                }
              >
                {item?.title}
              </NavLink>
      ...
      <div className="grid gap-x-[124px] grid-cols-[1.3fr,1fr] mb-[70px]">
        <div>
          ...
          <div className="w-full bg-white">
            <div
              className="text-center react-quill"

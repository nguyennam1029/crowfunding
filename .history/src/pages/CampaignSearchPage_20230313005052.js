...
import { getCampaign, setQuery } from "store/campaign/campaign-slice";
import { Button } from "components/button";

const CampaignSearchPage = () => {
  const dispatch = useDispatch();
  const { campaigns, query, limit } = useSelector((state) => state.campaign);
  console.log(limit);
  const { q } = useParams();
  useEffect(() => {
    dispatch(setQuery(q));
  ...
  return (
    <div>
      ...
      <Button kind="ghost" className="px-8 mx-auto" onClick={handleSeeMore}>
        <span>See more</span>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="w-5 h-5"
